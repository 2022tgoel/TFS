use crate::chunkserver::{CHUNK_SIZE, RpcMessage, PutInfo, SERVER_PORT};
...
impl RpcClient {
    pub fn new(local_addr: String, server_name: String) -> Result<Self> {
    ...
    pub async fn send_put_request(&mut self, file_id: u64, chunk_id: u64, data: &[u8]) -> Result<()> {
        println!("Sending PutRequest to {}", format!("{}:{}", self.server_name, SERVER_PORT));
        let mut stream = TcpStream::connect(format!("{}:{}", self.server_name, SERVER_PORT)).await?;
        ...
        println!("Connecting to {} from {}", self.server_name, self.local_addr);
        self.ib_socket
